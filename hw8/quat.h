...
Quat pow(const Quat& q, double expo);
...
inline Quat cn(const Quat& q) {
  return (q[0] < 0) ? Quat(-q[0], -q[1], -q[2], -q[3]) : q;
}

inline Quat pow(const Quat& q, double expo) {
  const Cvec3 k = Cvec3(q[1], q[2], q[3]);
  if (norm(k) < 1e-8)
    return Quat(1, 0, 0, 0);

  const double theta_half = atan2(norm(k), q[0]);
  const double new_theta = expo * theta_half;
  return Quat(cos(new_theta), normalize(k) * sin(new_theta));
}
#endif
