...
bool interpolateAndDisplay(float t) {
  ...
  for (int i = 0; i < g_animationState.numRbtNodes; i++) {
    RigTForm interpolateRbt = RigTForm(lerp(frame1[i].getTranslation(), frame2[i].getTranslation(), alpha)
                                    , slerp(frame1[i].getRotation(), frame2[i].getRotation(), alpha));
    curFrame.push_back(interpolateRbt);
  }

  for (int i = 0; i < g_animationState.nodePtrVec.size(); i++) {
